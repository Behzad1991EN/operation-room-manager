...
function verifyBackup(raw) {
  ...
  for (const [key, record] of Object.entries(next.schedules)) {
    ...
    if ([1, 2].includes(record?.input?.config?.version)) continue;
...
document.addEventListener('submit', async event => {
  ...
  try {
    if (form.getAttribute('id') === 'employee-form') {
      const raw = Object.fromEntries(data); raw.radiationBenefit = data.has('radiationBenefit'); raw.sectionSupervisor = data.has('sectionSupervisor');
